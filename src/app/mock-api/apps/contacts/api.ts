...
export class ContactsMockApi
{
    ...
    private _contacts_all: any[] = contactsData;
    ...
    registerHandlers(): void
    {
        ...
        this._fuseMockApiService
            ...
            .reply(() => {
                // Clone the contacts
                const contacts = cloneDeep(this._contacts_all);

                // Sort the contacts by the name field by default
                //contacts.sort((a, b) => a.orden.localeCompare(b.orden));
                contacts.sort((a, b) =>
                    a.orden - b.orden
                  );
            ...
            this._fuseMockApiService
            .onPost('api/apps/contacts/save-contact')
            .reply(({request}) => {
                // Get the id and contact
                const contacts: any = request;
            ...
            this._fuseMockApiService
            .onPost('api/apps/contacts/bycurso')
            .reply(({request}) => {
                // Get the search query
                //const query = request.params.get('id');
                let contacts = request.body;
                
                // If the query exists...
                /*if ( query )
                {
                    this._coursesService.getLessonByCurso(query)
                    .subscribe(
                        data => {
                        contacts = data;
                        this._contacts = data;
        ...
        this._fuseMockApiService
            ...
            .reply(({request}) => {
                ...
                {
                    // Filter the contacts
                    contacts = contacts.filter(contact => contact.nombre && contact.nombre.toLowerCase().includes(query.toLowerCase()));
                }
