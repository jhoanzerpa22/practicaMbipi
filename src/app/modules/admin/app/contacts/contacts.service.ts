...
import { Contact, Country, Tag } from 'app/modules/admin/app/contacts/contacts.types';
...
export class ContactsService
{
     ...
     * Get contacts
     */
     getContactsbyCurso(data: any): Observable<Contact[]>
     {  
        
        return this._httpClient.post<Contact[]>('api/apps/contacts/bycurso', 
            data
        ).pipe(
             tap((contacts) => {
                 this._contacts.next(contacts);
        ...
        /*return this._httpClient.get<Contact[]>(environment.API_G +`lecciones?curso_id=${id}`).pipe(
            tap((contacts) => {
                this._contacts.next(contacts);
            })
        );*/

        /*return this._httpClient.get<Contact[]>('api/apps/contacts/bycurso', {
            params: {id}
        }).pipe(
             tap((contacts) => {
                 this._contacts.next(contacts);
    ...
    searchContacts(query: string): Observable<Contact[]>
    {
        return this._httpClient.get<Contact[]>('api/apps/contacts/search', {
            ...
            tap((contacts) => {
                //console.log('resultado',contacts);
                this._contacts.next(contacts);
            })
        );
    }
    ...
    getContactById(id: any): Observable<Contact>
    {
        return this._contacts.pipe(
            ...
            map((contacts) => {
                ...
                let find: any = id.length < 6 ? parseInt(id) : id;
                // Find the contact
                //console.log('contacts_create',contacts);
                //console.log('find',find);
                const contact = contacts.find(item => item.id === find) || null;
